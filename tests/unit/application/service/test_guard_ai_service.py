...
class TestGuardAiService:
    ...
    def test_guard_ai_service_reaches_target_position(self):
        ...
        assert guard_ai_service.has_reach_target(guard.position, guard.patrol_points[guard.current_patrol_index], guard.patrol_radius) == True
    ...
    def test_guard_current_patrol_index_is_next_when_target_is_reached(self):
        ...
        guard_ai_service = GuardAiService(CollisionServiceImpl())
        map_obj = Map(10, 10)
        guard_ai_service.update_guard_position(guard, map_obj)
